...
namespace visulizare_project
{
    public partial class insertion_sort : Form
    {
        ...
        private void Form2_Load(object sender, EventArgs e)
        {
            ...
            project_name.Text = "Insertion Sort";
        ...
        private void insertion_sort_creation(object sender, EventArgs e)
        {
            ...
            else if (numbers.Text != null)
            {
                ...
                if (parsed)
                {
                    ...
                    else if (int.Parse(numbers.Text) <= 10 && int.Parse(numbers.Text) > 0)
                    {
                        ...
                        visualize.Controls.Add(line);
                        line.Hide();
                        Task.Factory.StartNew(() => insertion_sort_visulizar(arr, ar, x, line));
                        //  move(arr, ar, x);
                    }
                }
            }
        ...
        public void insertion_sort_visulizar(Button[] arr, Button[] ar, int x, Panel line)
        {

            line.Size = new Size(2, 50);
            line.BringToFront();
            line.BackColor = Color.Red;
            int key, j, m = 0;
            Button keyb = new Button();
            for (int i = 1; i < arr.Length; i++)
            {
                key = int.Parse(arr[i].Text);
                keyb = arr[i];
                j = i - 1;
                Task.Delay(TimeSpan.FromSeconds(t)).Wait();
                line.Location = new Point(arr[j].Location.X + 32, ((visualize.Height - arr[i].Height) / 2) - 10);
                line.Show();
                ...
                arr[i].FlatAppearance.BorderSize = 2;
                //  MessageBox.Show(" -1 " + arr[j + 1].ToString() + " " + arr[j].ToString());
                while (j >= 0 && int.Parse(arr[j].Text) > key)
                {
                    m++;
                    //   ar[j] = arr[j];
                    Task.Delay(TimeSpan.FromSeconds(t)).Wait();
                    ...
                    arr[j].Location = new Point(ar[j].Location.X, ((visualize.Height - arr[i].Height) / 2) + 40);
                    Task.Delay(TimeSpan.FromSeconds(t)).Wait();
                    arr[j].Location = new Point(ar[j + 1].Location.X, ((visualize.Height - arr[i].Height) / 2) + 40);
                    Task.Delay(TimeSpan.FromSeconds(t)).Wait();
                    arr[j].Location = new Point(ar[j + 1].Location.X, ((visualize.Height - arr[i].Height) / 2));
                ...
                //     MessageBox.Show(" 2 " + arr[i].ToString() + " : " + keyb.ToString());
                for (int k = 0; k < m; k++)
                {
                    Task.Delay(TimeSpan.FromSeconds(t)).Wait();
                    keyb.Location = new Point(ar[i - k - 1].Location.X, ((visualize.Height - arr[i].Height) / 2) - 40);
                }
                Task.Delay(TimeSpan.FromSeconds(t)).Wait();
                keyb.BackColor = Color.LightCoral;
                keyb.FlatAppearance.BorderColor = Color.Black;
                keyb.FlatStyle = FlatStyle.Flat;
                keyb.FlatAppearance.BorderSize = 2;
                keyb.BringToFront();
                keyb.Location = new Point(ar[i - m].Location.X, ((visualize.Height - arr[i].Height) / 2));
                arr[m - i] = keyb;
                //  MessageBox.Show(keyb.ToString());
                m = 0;
            }
        ...
        private void insertion_sort_FormClosing(object sender, FormClosingEventArgs e)
        {
            form.Show();
        }
    }
